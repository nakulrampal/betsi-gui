...
def regression_diagnostics_plots(bet_filtered, name, fig_2=None):
    ...
    sns.regplot(fit_values, fit_stud_resid_abs_sqrt,
                ...
                ci=False,
                lowess=True,
                line_kws={'color': 'black', 'lw': 1, 'alpha': .75}, ax=scale_loc)
    ...
    for i in abs_norm_resid_top_3:
        scale_loc.axes.annotate(i,
                                xy=(fit_values[i],
                                    fit_stud_resid_abs_sqrt[i]), size=11)
    ...
    sns.regplot(fit_leverage, fit_stud_resid,
                ...
                ci=False,
                lowess=True,
                line_kws={'color': 'black', 'lw': 1, 'alpha': .75},
                ax=res_vs_lev)
...
def plot_isotherm(bet_filtered, ax=None):
    ...
    ax.scatter(bet_filtered.pressure[:min_i],
               bet_filtered.q_adsorbed[:min_i],color='black', edgecolors='black', label='Adsorption Isotherm', alpha = 0.50)
    ax.scatter(bet_filtered.pressure[min_j+1:],
               bet_filtered.q_adsorbed[min_j+1:],color='black', edgecolors='black',alpha = 0.50)
    ...
    ax.scatter(bet_filtered.pressure[min_i:min_j + 1],
               ...
               marker='s',color='red', edgecolors='red', label='Linear Range', alpha = 0.5)
    ...
    ax.plot(bet_filtered.x_range,
            pchip_interpolate(bet_filtered.pressure, bet_filtered.q_adsorbed, bet_filtered.x_range), color='black',alpha=.75,
            label='Pchip Interpolation')
    # plot corresponding pressure
    ax.scatter(bet_filtered.corresponding_pressure_pchip[min_i, min_j], bet_filtered.nm[min_i, min_j], marker='^',
               color='blue',edgecolor='blue', label='$\mathregular{N_m}$ Read', alpha=0.50)

    # Plot selected Monolayer loading (single point)
    ax.scatter(bet_filtered.calc_pressure[min_i, min_j],
               bet_filtered.nm[min_i, min_j],
               marker='v', color='green', edgecolors='green',edgecolor='green', label='$\mathregular{N_m}$ BET')
...
def plot_roquerol_representation(bet_filtered, ax=None):
    ...
    ax.scatter(bet_filtered.pressure[:min_i],
               bet_filtered.rouq_y[:min_i], edgecolors='black', color='black', label=r'$\mathregular{N(1-P/P_0)}$', alpha=0.5)
    ax.scatter(bet_filtered.pressure[min_j+1:],
               bet_filtered.rouq_y[min_j+1:], edgecolors='black', color='black',alpha=0.5)
    # Plot the part corresponding to the linear region
    ax.scatter(bet_filtered.pressure[min_i:min_j + 1],
               bet_filtered.rouq_y[min_i:min_j + 1],
               marker='s', color='red', edgecolors='', label='Linear Range', alpha=0.50)
...
def plot_linear_y(bet_filtered, ax=None):
    ...
    ax.scatter(bet_filtered.pressure[min_i:min_j + 1],
               ...
               marker='s', color='r', edgecolors='red', alpha = 0.50)
    ...
    ax.plot([0, largest_valid_x],
            [intercept_at_opt, end_y], color='black',alpha=.75)
...
def plot_area_error_1(bet_filtered, ax=None, ax2=None):
    ...
    ax.scatter(bet_filtered.valid_knee_bet_areas,
               bet_filtered.valid_knee_pc_errors, color='b', edgecolors='b', marker='s', picker=5, alpha=0.5)
    ...
    ax2.scatter(x_coords_nonvalid, y_coords_nonvalid, color='r',
               edgecolors='r', picker=5, alpha = 0.5)
    ax2.scatter(bet_filtered.valid_knee_bet_areas,
               bet_filtered.valid_knee_pc_errors, color='b', edgecolors='b', marker='s', picker=5, alpha=.5)
    ax2.scatter(bet_filtered.bet_areas[min_i, min_j], bet_filtered.pc_error[min_i, min_j], marker='s', color='orange',
               edgecolors='orange')
...
def plot_monolayer_loadings(bet_filtered, ax=None):
    ...
    # Plot the Isotherm itself
    ax.scatter(bet_filtered.pressure[:min_i],
               bet_filtered.q_adsorbed[:min_i],
               color='black', edgecolors='black', alpha=0.5)
    ax.scatter(bet_filtered.pressure[min_j+1:],
               bet_filtered.q_adsorbed[min_j+1:],
               color='black', edgecolors='black', label='Adsorption Isotherm', alpha=0.5)
    # Plot the fitted spline.
    ax.plot(bet_filtered.x_range,
            pchip_interpolate(bet_filtered.pressure, bet_filtered.q_adsorbed, bet_filtered.x_range), color='black',alpha=.5,
            label='Pchip Interpolation')

    # Plot the valid monolayer loadings.
    ax.scatter(bet_filtered.valid_calc_pressures,
               bet_filtered.valid_nm,
               marker='^', color='blue', edgecolors='blue',
               label='$\mathregular{N_m}$ valid', alpha=0.5)
    ...
    ax.scatter(bet_filtered.calc_pressure[min_i, min_j],
               bet_filtered.nm[min_i, min_j],
               marker='s', color='orange', edgecolors='orange', label='N$_m$ BET')
...
def plot_box_and_whisker(bet_filtered, ax=None):
    ...
    medianprops = dict(linestyle='--', linewidth=1, color='black', alpha=0.35)
